...
Time series are expected as input data. The software handles 2D as well as 3D (z-stack) confocal images.
The software is intended to run on Windows 10/11.
...
Download python 3.* via windows app store
...
|---|---|
|**automated_colocalization_quantification** | Parent folder, can be anywhere, can be renamed|
...
|**your data folder pre analysis**|							| Add this folder yourself: It should contain data to be analyzed. The script can handle as many of them as you want, just add one folder for each time-series. Folder name can be changed. Must contain the files specified in the following lines, and nothing else:|
|filename_blue.tif							| Blue channel image data as a tif file. Axis order needs to be [t,y,x] or [t,z,y,x] (images must be a time-series). The filename is ideally the same for all three image files, except for the keywords (blue, green, nuclei OR blu3, gr33n, nucl3i). The filename can be anything, but shouldn't contain the key words more than once (i.e. the base filename shouldn't contain 'blue' or 'green' or 'nuclei').|
|filename_green.tif							| Green channel image data as a tif file. Axis order needs to be [t,y,x] or [t,z,y,x] (images must be a time-series). The filename is ideally the same for all three image files, except for the keywords (blue, green, nuclei OR blu3, gr33n, nucl3i). The filename can be anything, but shouldn't contain the key words more than once (i.e. the base filename shouldn't contain 'blue' or 'green' or 'nuclei').|
|filename_nuclei.tif							| Nucleus stain channel image data as a tif file. Axis order needs to be [t,y,x] or [t,z,y,x] (images must be a time-series). The filename is ideally the same for all three image files, except for the keywords (blue, green, nuclei OR blu3, gr33n, nucl3i). The filename can be anything, but shouldn't contain the key words more than once (i.e. the base filename shouldn't contain 'blue' or 'green' or 'nuclei').|
